...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "INSERT INTO pracownicy (nazwa_uzytkownika, haslo, email, stanowisko, pensja) 
            VALUES ('$nazwa_uzytkownika', '$haslo', '$email', '$stanowisko', '$pensja')";

    if ($conn->query($sql) === TRUE) {
        header("Location: pracownicy.php"); // Przekierowanie po dodaniu
        exit;
    } else {
        $error = "Błąd dodawania pracownika: " . $conn->error;
    }
}
?>
...
<head>
    ...
    <style>
        ...
        .card-header {
            ...
            color: white;
            text-align: center;
            border-border-radius: 1rem 1rem 0 0;
            padding: 1rem;
        }
        .card-body {
            padding: 1.5rem;
