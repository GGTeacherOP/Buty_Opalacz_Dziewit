...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($conn->query($sql) === TRUE) {
        header("Location: zarzadzaj_pracownikami.php"); // Przekierowanie po edycji
        exit;
    } else {
...
if (isset($_GET['id'])) {
    ...
    if ($result->num_rows == 1) {
        ...
        header("Location: zarzadzaj_pracownikami.php"); // Przekierowanie, jeśli nie znaleziono
        exit;
    }
} else {
    header("Location: zarzadzaj_pracownikami.php"); // Przekierowanie, jeśli brak ID
    exit;
}
?>
